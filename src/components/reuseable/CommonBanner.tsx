...
const CommonBanner: React.FC<CommonBannerProps> = ({
  ...
  return (
    <section
      ...
      className="w-full bg-gray-900 text-white py-16 md:py-40 relative overflow-hidden rounded-2xl md:rounded-4xl"
    >
      {/* Overlay (applied only to the background, not affecting the text) */}
      <div
        className={`absolute inset-0 bg-${overlayColor} opacity-20  rounded-2xl md:rounded-4xl`}
      ></div>

      {/* Banner Content */}
      <div className="relative z-10 text-center text-white">
        <h2 className="text-4xl sm:text-5xl md:text-[52px] font-medium font-inter max-w-[786px] mx-auto leading-relaxed md:leading-[150%] ">{title}</h2>
        <p className="text-lg md:text-xl px-3 font-medium font-inter leading-snug md:leading-[150%] mt-6 mb-10">{subtitle}</p>
        <a href={buttonLink}>
          <button  className="border border-white px-8 py-3 text-base font-normal leading-snug md:leading-[150%] font-inter hover:bg-white hover:text-gray-900 transition-all cursor-pointer">
            {buttonText}
          </button>
        </a>
      </div>
